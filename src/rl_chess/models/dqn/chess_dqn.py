...
def board_to_tensor(board: chess.Board):
    return torch.from_numpy(board_to_array(board=board)).to(ChessDQN.device).float()
...
class ChessDQN():
    ...
    def train(self, episodes: int,
        ...
        if keep_training == True:
            if file_path.exists():
                self.policy_dqn.load_state_dict(torch.load(file_path, map_location=ChessDQN.device, weights_only=True))
        ...
        for cycle in range(cycles):
            ...
            if self.verbose:
                if moves_optimized > 0:
                    print(f"Training time: {(toc-tic):.4f}, {((toc-tic)/moves_optimized):.4f} per move", end='\n')
                else:
    ...
    def test(self, episodes, verbose=True, file="chess_dqn.pt"):
        ...
        policy_dqn.load_state_dict(torch.load(file_path, map_location=ChessDQN.device, weights_only=True))
...
if __name__ == '__main__':
    chess_dqn = ChessDQN(device_type="cpu")  # "cpu" or "cuda", cpu works better using small models
    chess_dqn.train(episodes=25,        # episodes per cycle
                    cycles=1000,
